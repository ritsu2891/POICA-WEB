<template>
  <div class="d-flex justify-center align-center" style="height: 100%">
    <div class="pa-5" align="center">
      ...
      <p v-if="authRequiredMessage">
        <b>要求された操作を実行するために、ログインが必要です。</b>
      </p>
      <p>
        現在は<b>Googleアカウント</b>によるログイン・登録のみご利用いただけます。
      </p>
      <v-btn
        class="ma-2"
        tile
        color="primary"
        :loading="onAuthRequesting"
        @click="onGoogleLoginButtonClicked()"
      >
...
export default {
  ...
  methods: {
    onGoogleLoginButtonClicked: async function () {
      ...
      this.authProviderWindow = window.open(
  ...
  mounted() {
    this.authRequiredMessage = this.$cookies.get("authRequiredMessage")
      ? true
      : false;
